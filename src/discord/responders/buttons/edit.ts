        ...
        switch (action) {
            ...
            case "backup": {
                ...
                if (data.stock.length < 1) {
                    await interaction.followUp(res.primary("Desculpe, esse produto não possui estoque"));
                    return;
                }
            ...
            case "clear": {
                ...
                .then(async () => {
                    ...
                    await interaction.followUp(res.success("Parabéns, o estoque do produto foi limpo com sucesso"));
                })
                .catch(async (err) => {
                    log.error(err);
                    await interaction.followUp(res.danger("Desculpe, não foi possível limpar o estoque"));
                });
                return;
            }
            case "back": {
